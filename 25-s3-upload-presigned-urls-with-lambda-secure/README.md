...
However, we can enforce other restrictions such as how long the presigned URL is active for, the size range that the uploaded file must have, the file type, the file name among other things.  We accomplish this in the Lambda function's code with the JavaScript SDK's `createPresignedPost()`.
