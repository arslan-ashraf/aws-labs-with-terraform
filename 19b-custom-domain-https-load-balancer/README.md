...
We also create Route53 Public DNS A records the point a custom domain to the load balancer.  Finally, we also generate an HTTPS Certificate using Amazon Certificate Manager (ACM).  This way, we can access the website of the custom domain using HTTPS, that hits the load balancer.
