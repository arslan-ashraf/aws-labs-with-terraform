...
**Route53 Resolver Endpoints are expensive** because each endpoint creates an ENI for each subnet in which the resolver is created (`ip_address { ... }` block), hence this lab creates 4 ENIs and each ENI has a significant hourly cost.
