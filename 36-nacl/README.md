...
The code that is in the `user_data.sh` srcipt is all in the `app-server` directory.  We run a very simply web application on the EC2 instance and try to test if it is accessible from the user's local machine.