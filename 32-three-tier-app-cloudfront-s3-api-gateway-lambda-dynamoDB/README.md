...
In this lab, we create a three tier web application.  We start with a CloudFront distribution that will sit in front 

 API Gateway that sits in front of a Lambda function.  Further, we create an A record in the Route53 public zone that maps the custom domain name to the API Gateway.
